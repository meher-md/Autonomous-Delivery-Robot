...
# -*- coding: utf-8 -*-
"""
Gazebo + ROS 2 (Nav2) launch for Andino GZ, with multi-robot support and tidy structure.
- Spawns Gazebo world and optional ROS-GZ bridges
- Spawns one or more robots with namespaces
- Optionally launches RViz (software rendering) with/without Nav2
- Brings up Nav2 for single- or multi-robot setups
- Publishes a single static TF from gazebo_world -> map (identity), avoiding TF tree conflicts
- Optionally auto-publishes an initial pose and clears costmaps shortly after bringup
"""
import os
...
def generate_launch_description() -> LaunchDescription:
    ...
    world_name_arg = DeclareLaunchArgument(
        "world_name",
        default_value="populated_office.sdf",
    ...
    initpose_yaw_deg_arg = DeclareLaunchArgument(
        "initpose_yaw_deg",
        default_value="0.0",
        description="Initial yaw (degrees) in map frame.",
    )
    ...
    if robots_list == {}:
        ...
        robots_list = {"andino": {"x": 0.0, "y": 0.0, "z": 0.1, "yaw": 0.0}}
    log_number_robots = LogInfo(msg="Robots to spawn: " + str(robots_list))

    spawn_robots_group = []
    more_than_one_robot = PythonExpression(
        [TextSubstitution(text=str(len(robots_list.keys()))), " > 1"]
    )
    one_robot = PythonExpression(
        [TextSubstitution(text=str(len(robots_list.keys()))), " == 1"]
    )
    ...
    for robot_name, init_pose in robots_list.items():
        robots_group = GroupAction(
            ...
            actions=[
                ...
                PushRosNamespace(
                    condition=IfCondition(more_than_one_robot), namespace=robot_name
                ),
                # Spawn the robot
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                        os.path.join(
                            pkg_andino_gz, "launch", "include", "spawn_robot.launch.py"
                        )
                    ),
                    launch_arguments={
                        "entity": robot_name,
                ...
                # RViz with Nav2 (delayed + software rendering)
                TimerAction(
                    period=5.0,
                    actions=[
                        Node(
                            condition=IfCondition(
                                PythonExpression([rviz, " and ", LaunchConfiguration("nav2")])
                            ),
                            package="rviz2",
                            executable="rviz2",
                            arguments=[
                                "-d",
                                os.path.join(pkg_andino_gz, "rviz", "andino_gz_nav2.rviz"),
                            ],
                ...
                TimerAction(
                    ...
                    actions=[
                        Node(
                            condition=IfCondition(
                                PythonExpression(
                                    [rviz, " and not ", LaunchConfiguration("nav2")]
                                )
                            ),
                            package="rviz2",
                            executable="rviz2",
                            arguments=[
                                "-d",
                                os.path.join(pkg_andino_gz, "rviz", "andino_gz.rviz"),
                            ],
                            parameters=[{"use_sim_time": True}],
                            remappings=[("/tf", "tf"), ("/tf_static", "tf_static")],
                            output="screen",
                ...
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                        os.path.join(
                            pkg_andino_gz, "launch", "include", "gz_ros_bridge.launch.py"
                        )
                    ),
        ...
        nav_group = GroupAction(
            ...
            actions=[
                SetRemap(
                    src="/global_costmap/scan",
                    dst="/scan",
                    condition=IfCondition(
                        PythonExpression([one_robot, " and ", LaunchConfiguration("nav2")])
                    ),
                ),
                SetRemap(
                    src="/local_costmap/scan",
                    dst="/scan",
                    condition=IfCondition(
                        PythonExpression([one_robot, " and ", LaunchConfiguration("nav2")])
                    ),
                ...
                # Multi-robot
                SetRemap(
                    src=f"/{robot_name}/global_costmap/scan",
                    dst=f"/{robot_name}/scan",
                    condition=IfCondition(
                        PythonExpression(
                            [more_than_one_robot, " and ", LaunchConfiguration("nav2")]
                        )
                    ),
                ),
                SetRemap(
                    src=f"/{robot_name}/local_costmap/scan",
                    dst=f"/{robot_name}/scan",
                    condition=IfCondition(
                        PythonExpression(
                            [more_than_one_robot, " and ", LaunchConfiguration("nav2")]
