...
def generate_launch_description():
    ...
    # Keep world consistent with map by default
    ...
    # IMPORTANT: Build the FULL YAML path for map_server
    ...
    for robot_name in robots_list:
        ...
        nav_group = GroupAction(
            ...
            launch_configurations={
                'rviz': rviz,
                'ros_bridge': ros_bridge,
                # Pass FULL YAML path to Nav2 (critical)
                'map': map_path,
            ...
            actions=[
                ...
                # Nav2 bringup (wrapped in a small delay)
                TimerAction(
                    period=nav2_delay,
                    actions=[
                        # Single robot
                        IncludeLaunchDescription(
                            ...
                            launch_arguments={
                                'map': map_path,               # <- pass the YAML Substitution directly
                                'autostart': 'True',
                                'use_sim_time': 'True',
                                'params_file': LaunchConfiguration('params_file'),
                            }.items(),
