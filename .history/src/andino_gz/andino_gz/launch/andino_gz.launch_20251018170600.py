...
def generate_launch_description():
    ...
    # يصنع مسار: <pkg>/maps/<map>/<map>.yaml
    ...
    # ده بيكمل سلسلة TF: map -> odom (من AMCL) و odom(=world) -> base_link (من جازيبو)
    ...
    for robot_name in robots_list:
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                # RViz مع Nav2 (تأخير بسيط + Software Rendering)
                TimerAction(
                    period=5.0,
                    actions=[
                        Node(
                ...
                # RViz بدون Nav2 (أيضًا Software Rendering)
                TimerAction(
                    period=5.0,
                    actions=[
                        Node(
        ...
        nav_group = GroupAction(
            ...
            launch_configurations={
                'rviz': rviz,
                'ros_bridge': ros_bridge,
                'map': map_path,                     # المسار النهائي للـ YAML
                'params_file': params_file,
                'nav2': nav2_flag,
            },
            actions=[
                # Remap scan للسيناريو أحادي الروبوت
                ...
                # Remap للمالتي روبوت (بالـ namespace)
