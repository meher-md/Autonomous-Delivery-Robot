...
def generate_launch_description():
    ...
    base_group = GroupAction(
        ...
        actions=[
            ...
            # Bridge /clock only (do NOT bridge Pose->/tf here to avoid TF conflicts)
            Node(
                package='ros_gz_bridge',
                executable='parameter_bridge',
                # Clock is enough here; other bridges are brought per-robot below
    ...
    for robot_name in robots_list:
        ...
        # ---------------- Per-robot group: spawn, RViz, bridges ----------------
        robots_group = GroupAction(
            ...
            actions=[
                ...
                # ---------------- Bridges (per robot) ----------------
                # NOTE:
                # 1) We bridge Odometry to /odom (nav_msgs/Odometry).
                # 2) We bridge sensors (scan, image, camera_info, joint_states, imu).
                # 3) We DO NOT bridge Pose_V -> /tf here to avoid a competing TF tree from 'world'.
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                        os.path.join(pkg_andino_gz, 'launch', 'include', 'gz_ros_bridge.launch.py')
                    ),
                    launch_arguments={'entity': robot_name, 'bridge_pose_tf': 'false'}.items(),
                    condition=IfCondition(ros_bridge),
                ),
            ]
        )
