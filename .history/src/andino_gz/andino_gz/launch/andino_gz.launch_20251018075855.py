...
def generate_launch_description():
    ...
    # ---------------- Static TF: map -> world (تصحيح اتجاه الـ TF) ----------------
    ...
    for robot_name in robots_list:
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                # RViz (Nav2 config) - Software GL لتقليل مشاكل OpenGL داخل الحاويات
                TimerAction(
                    period=5.0,
                    actions=[
                        Node(
        ...
        nav_group = GroupAction(
            ...
            actions=[
                ...
                # EKF: ينشر odom->base_link من /odom و /imu/data (تم ضبط الكونفيج)
                Node(
...
def generate_launch_description():
    ...
    # ملاحظة: جعل العالم الافتراضي يطابق الخريطة الافتراضية لتجنّب عدم التوافق
    world_name_arg = DeclareLaunchArgument(
    ...
    # تفعيل استخدام وقت المحاكاة عالميًا منذ البداية (يقلل أخطاء Extrapolation)
    set_sim_time_globally = SetParameter(name='use_sim_time', value=True)

    # تحسين عرض RViz داخل الحاويات
    sw_gl_env = SetEnvironmentVariable('LIBGL_ALWAYS_SOFTWARE', '1')
    ...
    base_group = GroupAction(
        ...
        actions=[
            ...
            # ROS Bridge للـ /clock فقط (جسور الحساسات تُطلق لاحقًا)
            Node(
                package='ros_gz_bridge',
                executable='parameter_bridge',
                arguments=['/clock@rosgraph_msgs/msg/Clock[ignition.msgs.Clock'],
    ...
    static_tf_map_to_world = Node(
        ...
        executable='static_transform_publisher',
        name='tf_map_to_world',
        # x y z roll pitch yaw parent child (هوية)
        arguments=['0', '0', '0', '0', '0', '0', 'map', 'world'],
        output='screen'
    )
    ...
    # لتقليل مشاكل الـ TF/Clock: نؤخر تشغيل Nav2 & RViz بعد تشغيل المحاكاة قليلًا
    nav2_delay_sec = 8.0
    rviz_delay_sec = 5.0
    ...
    for robot_name in robots_list:
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                # Bridges بين Gazebo و ROS (حساسات، محركات..)
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                        os.path.join(pkg_andino_gz, 'launch', 'include', 'gz_ros_bridge.launch.py')
                    ),
        ...
        nav_group = GroupAction(
            ...
            launch_configurations={
                'rviz': rviz,
                'ros_bridge': ros_bridge,
                'map': map_path,          # نمرّر المسار الكامل للخريطة داخل هذا الجروب
                'params_file': params_file,
                'nav2': nav2_flag,
            },
            actions=[
                ...
                # Nav2 bringup (مؤجّل قليلًا لضمان جهوزية الـ TF والـ Clock)
                TimerAction(
                    ...
                    actions=[
                        IncludeLaunchDescription(
                            ...
                            launch_arguments={
                                'map': LaunchConfiguration('map'),     # سيأخذ قيمة map_path من launch_configurations
                                'autostart': 'True',
                                'use_sim_time': 'True',
                                'params_file': LaunchConfiguration('params_file'),
                            }.items(),
    ...
    # ---------------- Auto initial pose & clear costmaps ----------------
    # ننشر initialpose بعد التأكد من تشغيل Nav2/TF (بعد 10 ثواني تقريبًا)
    auto_initial_pose = TimerAction(
        period=10.0,
        actions=[
            ExecuteProcess(
    ...
    # مسح الكوستماب بعد نشر الوضع الابتدائي مباشرة
    clear_costmaps = TimerAction(
        period=12.0,
        actions=[
            ExecuteProcess(
