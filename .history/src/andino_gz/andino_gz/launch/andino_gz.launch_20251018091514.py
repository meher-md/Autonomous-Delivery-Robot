...
def generate_launch_description():
    ...
    robots_arg     = DeclareLaunchArgument(
        ...
        description='Robots to spawn; separate multiple entries with semicolons.'
    )
    ...
    base_group = GroupAction(
        actions=[
            # Avoid GL issues inside containers
            ...
            # Bridge only the /clock topic (required for use_sim_time)
            Node(
    ...
    # ---------------- Static TF: map → world ----------------
    # Small offset can be used if the world and map origins are not perfectly aligned.
    static_tf_map_to_world = Node(
        package='tf2_ros', executable='static_transform_publisher', name='tf_map_to_world',
        arguments=['0.35', '-0.12', '0', '0', '0', '0.03', 'map', 'world'], output='screen'
    )
    # No robot_localization is used here.
    # The odom→base_link TF comes from Gazebo (diff_drive plugin),
    # and map→odom from AMCL inside Nav2.
    ...
    for robot_name, init_pose in robots_list.items():
        ...
        robot_group = GroupAction(
            actions=[
                ...
                # Robot State Publisher (for robot link TFs)
                Node(
        ...
        # Nav2 setup (planning, localization, controller)
        nav_group = GroupAction(
            actions=[
                # Remap LIDAR scan topics to costmaps
    ...
    # ---------------- Automatic Initial Pose and Costmap Clearing ----------------
    ...
    # Auto initial pose + costmap clearing
