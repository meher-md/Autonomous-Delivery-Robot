...
def generate_launch_description():
    ...
    # مهم: خلّي الافتراضي gazebo_world علشان التطابق مع واجهة جازيبوا
    ...
    # ---------------- Static TF ----------------
    # دقّة: نوحّد الشجرة بدون أي loops
    ...
    for robot_name in robots_list:
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                # RViz مع Nav2 (fixed frame = map لضمان التطابق + software GL)
                TimerAction(
                    period=5.0,
                    actions=[
                        Node(
                            ...
                            package='rviz2',
                            executable='rviz2',
                            # -f map يضمن أن الفريم الأساسي Map
                ...
                # RViz بدون Nav2 (برضه Fixed Frame = map)
                TimerAction(
                    period=5.0,
                    actions=[
                        Node(
                            ...
                            package='rviz2',
                            executable='rviz2',
                            arguments=['-f', 'map', '-d', os.path.join(pkg_andino_gz, 'rviz', 'andino_gz.rviz')],
                            parameters=[{'use_sim_time': True}],
                            remappings=[('/tf', 'tf'), ('/tf_static', 'tf_static')],
                            output='screen',
                            additional_env={
                ...
                # Gazebo<->ROS bridges الخاصة بالروبوت
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
        ...
        nav_group = GroupAction(
            ...
            actions=[
                # Remaps للـ scan حسب عدد الروبوتات
    ...
    auto_initial_pose = TimerAction(
        period=10.0,  # بعد ما Gazebo/Nav2/TF يبقوا جاهزين
        actions=[
            ExecuteProcess(
                cmd=[
                    'python3', '-c',
    ...
    # نمسح الكوست مابس بعد ضبط الـ initialpose علشان تتحدّث
    ...
    clear_costmaps_timer = TimerAction(
        period=12.0,  # بعد الـ initialpose بقليل
    ...
    # static TF (gazebo_world -> odom افتراضيًا)
