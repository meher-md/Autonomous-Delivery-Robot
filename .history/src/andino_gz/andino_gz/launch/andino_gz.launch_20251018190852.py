...
def generate_launch_description():
    ...
    # Which "world-like" parent frame owns odom
    tf_world_parent_arg = DeclareLaunchArgument(
        'tf_world_parent', default_value='gazebo_world',
        description='Frame to use as parent for odom (e.g., gazebo_world or world).'
    )

    # Initial pose arguments (map frame)
    initpose_x_arg     = DeclareLaunchArgument('initpose_x', default_value='0.0', description='Initial pose X in map frame.')
    initpose_y_arg     = DeclareLaunchArgument('initpose_y', default_value='0.0', description='Initial pose Y in map frame.')
    initpose_yaw_deg_arg = DeclareLaunchArgument('initpose_yaw_deg', default_value='0.0', description='Initial yaw (degrees) in map frame.')
    ...
    # ---------------- Static TF (single world->odom) ----------------
    # English: single fixed transform parent(tf_world_parent) -> odom to avoid loops
    static_tf_world_to_odom = Node(
        package='tf2_ros', executable='static_transform_publisher', name='tf_world_to_odom',
    ...
    for robot_name in robots_list:
        ...
        # ========== Bridges (NO Pose_V->tf), publish odom to /odom_raw ==========
        per_robot_bridge = Node(
            package='ros_gz_bridge', executable='parameter_bridge', output='screen',
            arguments=[
                f'/model/{robot_name}/odometry@gz.msgs.Odometry@nav_msgs/msg/Odometry',
                f'/world/gazebo_world/model/{robot_name}/link/base_link/sensor/sensor_ray_front/scan'
                f'@gz.msgs.LaserScan@sensor_msgs/msg/LaserScan',
                '/imu/data@gz.msgs.IMU@sensor_msgs/msg/Imu',
                f'/model/{robot_name}/cmd_vel@geometry_msgs/msg/Twist@gz.msgs.Twist'
            ],
            # Send odom to /odom_raw; scan to /scan; cmd_vel to /cmd_vel
            remappings=[
                (f'/model/{robot_name}/odometry', '/odom_raw'),
                (f'/world/gazebo_world/model/{robot_name}/link/base_link/sensor/sensor_ray_front/scan', '/scan'),
                (f'/model/{robot_name}/cmd_vel', '/cmd_vel'),
            ],
            condition=IfCondition(ros_bridge),
        )

        # ========== Lightweight reframers ==========
        # English: Reframe Odometry: set header.frame_id='odom', child_frame_id='base_link' and republish on /odom
        odom_reframer = ExecuteProcess(
            cmd=[
                'python3', '-u', '-c',
                (
                    ...
                    '    msg.child_frame_id=msg.child_frame_id or "base_link"; '
                    '    if msg.child_frame_id != "base_link": msg.child_frame_id="base_link"; '
                    '    self.pub.publish(msg); '
                    'r=R(); '
                    'try: rclpy.spin(r) '
                    'finally: r.destroy_node(); rclpy.shutdown()'
                )
            ],
            output='screen'
        )

        # English: Ensure IMU frame_id is base_link (if sim publishes something else)
        imu_reframer = ExecuteProcess(
            cmd=[
                'python3', '-u', '-c',
                (
                    'import rclpy; rclpy.init(); '
        ...
        ekf_params = {
            ...
            # Inputs (after reframing)
            'odom0': '/odom',
            # English: fuse X,Y position + yaw + linear/angular vel; ignore Z & roll/pitch
        ...
        ekf_node = Node(
            ...
            name=f'ekf_odom_{robot_name}', output='screen', parameters=[ekf_params],
        )
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                # Bridges + reframers + EKF
                per_robot_bridge,
                odom_reframer,
                imu_reframer,
                ekf_node,

                # RViz with Nav2 (software rendering)
                TimerAction(
                    period=5.0,
                    actions=[
                        Node(
        ...
        nav_group = GroupAction(
            ...
            actions=[
                # Remap scan topics (single robot)
    ...
    clear_costmaps = GroupAction(actions=[
    ...
    clear_costmaps_timer = TimerAction(period=12.0, actions=[clear_costmaps], condition=IfCondition(nav2_flag))
    ...
    ld.add_action(tf_world_parent_arg)
    ...
    ld.add_action(clear_costmaps_timer)
