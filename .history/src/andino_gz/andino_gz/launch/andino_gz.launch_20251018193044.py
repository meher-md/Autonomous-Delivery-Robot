...
def generate_launch_description():
    ...
    # Choose the world-like parent for /odom.
    # NOTE: Many ros_gz worlds use 'world' (not 'gazebo_world'), so we default to 'world'
    # to guarantee that the TF tree has: world -> odom -> ... -> base_link (EKF publishes odom->base_link).
    tf_world_parent_arg = DeclareLaunchArgument(
        'tf_world_parent',
        default_value='world',
        description='Frame to use as parent for odom (e.g., world or gazebo_world).'
    )
    ...
    base_group = GroupAction(
        ...
        actions=[
            ...
            # ROS Bridge for /clock (fixed spec)
            Node(
    ...
    # Provide exactly ONE transform: <world-like> -> odom. Do NOT add any odom->world.
    ...
    # Static TF: ONLY world-like -> odom
