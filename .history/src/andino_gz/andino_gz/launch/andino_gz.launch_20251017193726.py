...
def generate_launch_description():
    ...
    gui_config_arg = DeclareLaunchArgument('gui_config', default_value='default.config', description='Ign GUI config')
    nav2_arg = DeclareLaunchArgument('nav2', default_value='True', description='Enable Nav2 Bringup.')
    map_name_arg = DeclareLaunchArgument('map', default_value="office", description='Map name (without ext)')
    ...
    # ---------------- TF Fixes ----------------
    # اربط عالم الجازيبو بإطار الخريطة مباشرة بعلاقة ثابتة صفرية
    tf_map_to_gz = Node(
        package='tf2_ros', executable='static_transform_publisher', name='tf_map_to_gazebo_world',
        arguments=['0', '0', '0', '0', '0', '0', 'map', 'gazebo_world'], output='screen'
    )
    # ولو فيه frame اسمه world اربطه برضه بالخريطة (صفر)
    tf_map_to_world = Node(
...
'''
    auto_initial_pose = TimerAction(
        period=10.0,  # wait Nav2 + map_server + amcl
        actions=[
            ExecuteProcess(
                cmd=['python3', '-c', initial_pose_script, initpose_x, initpose_y, initpose_yaw_deg],
                output='screen'
            )
        ]
    )

    # ---------------- Clear costmaps (with timeout-friendly waits) ----------------
    clear_costmaps_script = r'''
...
def call_once(node: Node, name: str, max_wait=30.0):
    cli = node.create_client(Empty, name)
    waited = 0.0
    while not cli.wait_for_service(timeout_sec=1.0):
        if stop:
            return
        ...
        if waited >= max_wait:
            node.get_logger().warn(f"Timeout waiting for {name}, skipping.")
            return
    req = Empty.Request()
    fut = cli.call_async(req)
    rclpy.spin_until_future_complete(node, fut)
    node.get_logger().info(f"Called {name}")

def main():
    rclpy.init()
    n = Node("clear_costmaps_once")
    call_once(n, "/global_costmap/clear_entirely_global_costmap")
    call_once(n, "/local_costmap/clear_entirely_local_costmap")
    rclpy.shutdown()
...
'''
    clear_costmaps = TimerAction(
        ...
        actions=[
            ExecuteProcess(
                cmd=['python3', '-c', clear_costmaps_script],
                output='screen'
            )
        ]
    )
    ...
    ld.add_action(LogInfo(msg="Robots provided & processed."))
    ...
    # TF fixes (تربط gazebo_world/world مع map)
    ld.add_action(tf_map_to_gz)
    ld.add_action(tf_map_to_world)
