...
def generate_launch_description():
    ...
    # Quote the map name inside PythonExpression to avoid NameError and strip optional ".yaml"
    map_stem = PythonExpression(["'", map_name, "'.endswith('.yaml') and '", map_name, "'[:-5] or '", map_name, "'"])
    map_path = PathJoinSubstitution([pkg_andino_gz, 'maps', map_stem, TextJoin(substitutions=[map_stem, '.yaml'])])
