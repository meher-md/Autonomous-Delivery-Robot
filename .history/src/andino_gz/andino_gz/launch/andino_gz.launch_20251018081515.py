...
def generate_launch_description():
    ...
    # Keep world and map consistent by default
    world_name_arg = DeclareLaunchArgument(
        'world_name', default_value='office.sdf',
        description='World file to load (should match the chosen map).'
    )
    ...
    map_name_arg = DeclareLaunchArgument(
        ...
        description='Map directory name under andino_gz/maps/.'
    )
    ...
    # Build the *full* YAML path; this is what Nav2 map_server expects
    ...
    base_group = GroupAction(
        ...
        actions=[
            # Force software GL inside containers for RViz stability
    ...
    # ---------------- Static TF: make map and world the same (identity) ----------------
    # This avoids splitting the TF tree. Gazebo's "world" and Nav2's "map" are aligned.
    ...
    for robot_name in robots_list:
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                # Robot State Publisher (needed for sensor/link TFs)
                Node(
                ...
                # RViz with Nav2 config (Software GL)
                TimerAction(
                    period=rviz_delay,
                    actions=[
                        Node(
        ...
        nav_group = GroupAction(
            ...
            launch_configurations={
                'rviz': rviz,
                'ros_bridge': ros_bridge,
                'map': map_path,            # <-- pass full YAML path to Nav2
            ...
            actions=[
                ...
                # EKF: publish odom -> base_link from /odom and /imu/data
                Node(
                ...
                # Nav2 bringup wrapped in a small delay so TF/clock are ready
                TimerAction(
                    period=nav2_delay,
                    actions=[
                        # Single robot
    ...
    # Publish initial pose after Nav2 is up so AMCL can create map->odom
    auto_initial_pose = TimerAction(
        period=10.0,  # ~after Nav2 + TF are ready
        actions=[
            ExecuteProcess(
                cmd=[
                    'python3', '-c',
