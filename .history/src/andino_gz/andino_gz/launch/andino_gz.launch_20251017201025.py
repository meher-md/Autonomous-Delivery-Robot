...
def generate_launch_description():
    ...
    # ---------- Force localhost-only (fix network/hotspot issues) ----------
    localhost_env = [
        SetEnvironmentVariable('ROS_LOCALHOST_ONLY', '1'),
        # Gazebo Transport envs (Garden/Fortress -> GZ_, older -> IGN_)
    ...
    # ---------------- Static TF (map <-> gazebo_world) ----------------
    static_tf_gz2map = Node(
        package='tf2_ros',
    ...
    # ---------------- Auto initial pose (repeat for a short burst) ----------------
    auto_initial_pose = TimerAction(
        period=18.0,  # wait a bit longer to ensure AMCL active
        actions=[
            ExecuteProcess(
                cmd=[
                    'python3','-c',
                    (
                        ...
                        't_end=time.time()+2.5; '      # انشر لمدة ~2.5 ثانية
    ...
    # Static TF fix
    ld.add_action(static_tf_gz2map)
