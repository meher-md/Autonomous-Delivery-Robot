...
# IMPORTANT: keep this helper to build strings from substitutions
...
def generate_launch_description():
    ...
    # Logs (must use TextJoin so substitutions are rendered, not repr())
    ...
    # ---------------- Gazebo Sim ----------------
    # Use TextJoin to build the exact string expected by ros_gz_sim/gz_sim.launch.py
    gz_args = TextJoin(
        substitutions=[
            world_path,
    ...
    # Keep ONLY this static TF. Do NOT publish odom->world etc.
    ...
    for robot_name, pose in robots_list.items():
        ...
        group_spawn = GroupAction(
            ...
            actions=[
                ...
                # RViz (software rendering; avoids Wayland/GL segfaults)
                TimerAction(
                    period=5.0,
                    actions=[
                        Node(
        ...
        group_nav2 = GroupAction(
            ...
            actions=[
                ...
                # Scan remaps (multi-robot, namespaced)
                SetRemap(
    ...
    # ---------------- Auto initial pose (force FastRTPS to avoid Cyclone conflict) ----------------
