...
def generate_launch_description():
    ...
    robots_arg     = DeclareLaunchArgument('robots', default_value="andino={x: 0., y: 0., z: 0.1, yaw: 0.};",
                                           description='Robots to spawn; separate multiple robots by ";"')
    ...
    map_name_arg   = DeclareLaunchArgument('map', default_value="office",
                                           description='Name of the map folder (maps/<name>/<name>.yaml)')
    ...
    gz_args = TextJoin(
        substitutions=[
            ...
            # يمكنك إضافة "--force-version 6" لو منصّب Ignition Gazebo Garden
        ],
        separator=' ',
    )
    ...
    # ---------------- Static TF: gazebo_world -> map (هوية) ----------------
    ...
    # args
    ld.add_action(ros_bridge_arg); ld.add_action(rviz_arg); ld.add_action(world_name_arg); ld.add_action(robots_arg)
    ld.add_action(gui_config_arg); ld.add_action(nav2_arg); ld.add_action(map_name_arg); ld.add_action(params_file_arg)
    ld.add_action(initpose_x_arg); ld.add_action(initpose_y_arg); ld.add_action(initpose_yaw_deg_arg)

    # logs
    ld.add_action(log_robots_by_user); ld.add_action(LogInfo(msg="Robots to spawn: " + str(robots_list)))
    ...
    # Static TF fix
    ld.add_action(static_tf_gz_to_map)

    # Robots + Nav2
    for grp in spawn_robots_group:
        ld.add_action(grp)
