...
def generate_launch_description():
    ...
    ros_bridge_arg = DeclareLaunchArgument(
        'ros_bridge', default_value='True', description='Run ROS–Gazebo bridge (clock only).'
    )
    ...
    # ---------------- Gazebo ----------------
    # مهم: لا نعمل bridge لـ /tf من جازيبو إطلاقًا. فقط /clock.
    gz_args = TextJoin(
        substitutions=[
            world_path,
    ...
    base_group = GroupAction(
        actions=[
            ...
            # Bridge /clock from Gazebo to ROS (بدون /tf)
            Node(
    ...
    # نجعل map->world = origin من ملف الـ YAML (لو موجود) أو هوية.
    def _make_static_tf(context):
        ...
        if align_mode == 'use_map_origin':
            try:
                ...
                origin = info.get('origin', [0.0, 0.0, 0.0])  # [x, y, yaw]
    ...
    for robot_name, init_pose in robots_list.items():
        ...
        # prefix للخدمات عند التعدد
        ns_prefix = ('/' + robot_name) if nrobots > 1 else ''
        ...
        robot_group = GroupAction(
            actions=[
                ...
                # Per-robot Gazebo↔ROS bridges (لا نعبر /tf)
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
        ...
        nav_group = GroupAction(
            actions=[
                        ...
                        ExecuteProcess(
                            ...
                            cmd=[
                                ...
                                (
                                    ...
                                    'rclpy.init(); ' +
                                    f'n=Node("auto_initialpose_{robot_name}"); ' +
                                    'p=n.create_publisher(PoseWithCovarianceStamped,"' +
                                    (ns_initialpose_topic if nrobots > 1 else '/initialpose') +
                                    '",10); ' +
                                    f'x={float(init_pose["x"])}; y={float(init_pose["y"])}; yaw={float(init_pose["yaw"])}; ' +
                                    ...
                                    'p.publish(m); '
                                    'n.get_logger().info(f"[auto_initialpose] x={x}, y={y}, yaw(rad)={yaw:.3f}"); '
                                    'rclpy.shutdown()'
                                ),
                            ],
                            output='screen'
                ...
                # One-shot clear costmaps shortly بعد الـ pose
                TimerAction(
                    period=nav2_delay + 4.0,
                    actions=[
                        ExecuteProcess(
                            cmd=['/bin/bash','-lc',
                                 f'ros2 service call {ns_prefix}/global_costmap/clear_entirely_global_costmap std_srvs/srv/Empty "{{}}"'],
                            output='screen'
                        ),
                        ExecuteProcess(
                            cmd=['/bin/bash','-lc',
                                 f'ros2 service call {ns_prefix}/local_costmap/clear_entirely_local_costmap std_srvs/srv/Empty "{{}}"'],
                            output='screen'
                        ),
                    ]
                ),
    ...
    # TF: map بالنسبة لـ world (من الـ YAML أو هوية)
