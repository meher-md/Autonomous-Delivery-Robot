...
def generate_launch_description():
    ...
    # Static TF (gazebo_world -> map) defaults from your /amcl_pose
    map_tf_x_arg        = DeclareLaunchArgument('map_tf_x',   default_value='0.5380513498')
    map_tf_y_arg        = DeclareLaunchArgument('map_tf_y',   default_value='0.3184176279')
    map_tf_yaw_arg      = DeclareLaunchArgument('map_tf_yaw', default_value='0.0093462452')  # rad
    ...
    tf_x        = LaunchConfiguration('map_tf_x')
    tf_y        = LaunchConfiguration('map_tf_y')
    tf_yaw      = LaunchConfiguration('map_tf_yaw')
    ...
    # Keep the first robot's spawn pose (not used now for offsets, but kept in case needed)
    ...
    static_tf_node = Node(
        ...
        arguments=[
            tf_x, tf_y, '0',           # x y z
            '0', '0', tf_yaw,          # roll pitch yaw
            'gazebo_world', 'map'      # parent child
        ],
        output='screen'
    )

    # ----------------- Initial Pose at (0,0,0) in map (after map_server & amcl active) -----------------
...
n.get_logger().info("/initialpose (map) set to (0, 0, yaw=0)")
n.destroy_node(); rclpy.shutdown()
'''

    auto_align_and_init = TimerAction(
        period=12.0,  # give time for bringup to reach 'active'
        actions=[
            ExecuteProcess(
    ...
    for a in (ros_bridge_arg, rviz_arg, world_name_arg, robots_arg,
              ...
              map_tf_x_arg, map_tf_y_arg, map_tf_yaw_arg):
        ld.add_action(a)
    ...
    # static TF then init pose then (optionally) clear costmaps
