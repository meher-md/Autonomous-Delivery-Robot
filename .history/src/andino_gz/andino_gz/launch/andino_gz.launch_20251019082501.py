...
def generate_launch_description():
    ...
    # Offsets to align Gazebo spawn with map (defaults set from your /amcl_pose)
    offset_x_arg        = DeclareLaunchArgument('map_offset_x', default_value='0.0062972443')
    offset_y_arg        = DeclareLaunchArgument('map_offset_y', default_value='0.0188892018')
    offset_yaw_arg      = DeclareLaunchArgument('map_offset_yaw', default_value='0.003113354')
    ...
    # ----------------- Initial Pose with user offsets (no static TF) -----------------
...
n.get_logger().info(f"/initialpose (map) = (spawn {SPAWN_X:.3f},{SPAWN_Y:.3f}) + off ({OFF_X:.3f},{OFF_Y:.3f}) -> ({x_map:.3f},{y_map:.3f}), yaw={yaw_map:.6f}")
...
'''
    ...
    for a in (ros_bridge_arg, rviz_arg, world_name_arg, robots_arg,
              ...
              offset_x_arg, offset_y_arg, offset_yaw_arg):
        ld.add_action(a)
