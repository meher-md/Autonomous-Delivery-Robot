...
def generate_launch_description():
    ...
    ros_bridge_arg = DeclareLaunchArgument(
        'ros_bridge', default_value='True',
        description='Run ROS bridge node.'
    )
    ...
    world_name_arg = DeclareLaunchArgument(
        ...
        description='Name of the world to load. Must match the map if using Nav2.'
    )
    map_name_arg = DeclareLaunchArgument(
        'map', default_value='office',
        description='Map folder/name to use with Nav2.'
    )
    ...
    # EKF params (to convert Odometry -> TF odom->base_link)
    ekf_params_file_arg = DeclareLaunchArgument(
        'ekf_params_file',
        default_value=PathJoinSubstitution([pkg_andino_gz, 'config', 'ekf.yaml']),
        description='EKF configuration file (robot_localization).'
    )

    # Initial pose in map frame (used to seed AMCL)
    ...
    # ---------------- Gazebo ----------------
    # Use software rendering in RViz later to sidestep EGL issues.
    gz_args = TextJoin(
        substitutions=[
            world_path,
    ...
    base_group = GroupAction(
        ...
        actions=[
            ...
            # Bridge /clock only (do NOT bridge Pose_V->/tf here to avoid TF conflicts)
            Node(
    ...
    # Connect Gazebo 'world' frame with Nav2 expected 'odom' tree.
    ...
    for robot_name in robots_list:
        ...
        # ---------------- Per-robot group: spawn, RViz, bridges, EKF ----------------
        robots_group = GroupAction(
            ...
            launch_configurations={
                ...
                'ros_bridge': ros_bridge,
                'nav2': nav2_flag,
                'ekf_params_file': ekf_params_file   # ensure EKF params are visible in this scope
            },
            actions=[
                LogInfo(msg="Group for robot: " + robot_name),

                # Namespace only if multiple robots
                ...
                # RViz (with Nav2 config) — software rendering to avoid EGL issues
                TimerAction(
                    period=5.0,
                    actions=[
                        Node(
                ...
                # ---------------- Bridges (per robot) ----------------
                # We bridge Odometry and sensors only; NO Pose_V->/tf to avoid a competing TF tree.
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                    ...
                    launch_arguments={
                        'entity': robot_name,
                        'bridge_pose_tf': 'false'  # critical: do not bridge TF from Gazebo
                    }.items(),
                ),

                # ---------------- EKF (robot_localization) ----------------
                # Consumes /odom and publishes TF: odom -> base_link
                Node(
                    package='robot_localization',
                    executable='ekf_node',
                    name='ekf_odom',              # must match the YAML key
                    output='screen',
    ...
    ld.add_action(map_name_arg);   ld.add_action(params_file_arg); ld.add_action(ekf_params_file_arg)
    ...
    # Base (Gazebo + /clock bridge)
    ld.add_action(base_group)

    # Static TF
    ld.add_action(LogInfo(msg="Static TF world->odom (identity)"))
    ld.add_action(world_to_odom_tf)

    # Robots + Nav2
    ld.add_action(LogInfo(msg="Spawning robots and Nav2..."))
