...
def generate_launch_description():
    ...
    base_group = GroupAction(
        ...
        actions=[
            ...
            # FIX: close the missing bracket in the bridge type spec
            Node(
    ...
    # ---------------- Static TF (connect world-related frames) ----------------
    # IMPORTANT:
    # Do NOT create a world <-> odom loop. Many stacks (Gazebo / other launch files)
    # may already publish a transform that implies world->odom. Publishing odom->world
    # here introduces a cycle (world parent odom AND odom parent world).
    #
    # Keep ONLY this helper to relate Gazebo's internal world to a child frame (optional).
    static_tf_gz = Node(
        package='tf2_ros',
        executable='static_transform_publisher',
        name='tf_odom_to_gazebo_world',
        arguments=['0', '0', '0', '0', '0', '0', 'odom', 'gazebo_world'],
        output='screen'
    )

    # REMOVED: this was causing the TF loop with external publishers
    # static_tf_world = Node(
    #     package='tf2_ros',
    #     executable='static_transform_publisher',
    #     name='tf_odom_to_world',
    ...
    # Static TF fix (ONLY keep gazebo_world relation; DO NOT add odom<->world)
    ld.add_action(static_tf_gz)
    # ld.add_action(static_tf_world)  # Removed to avoid TF loop
