...
def generate_launch_description():
    ...
    map_path        = PathJoinSubstitution([pkg_andino_gz, 'maps', map_name, TextJoin(substitutions=[map_name, '.yaml'], separator='')])

    # Logs (use TextJoin so substitutions render to strings)
    ...
    # ---------------- Environment (LOCAL ONLY + safer GL) ----------------
    ...
    for robot_name, pose in robots_list.items():
        ...
        group_spawn = GroupAction(
            ...
            actions=[
                ...
                # Per-robot Gazebo<->ROS bridges (your include handles odom/scan/etc.)
                IncludeLaunchDescription(
        ...
        group_nav2 = GroupAction(
            ...
            actions=[
                ...
                IncludeLaunchDescription(
                    ...
                    condition=IfCondition(PythonExpression([LaunchConfiguration('nav2')])),
                ),
            ]
        )
    ...
    # ---------------- Odometry -> TF relay (FIX: publish odom->base_link) ----------------
    # Converts nav_msgs/Odometry to TF to satisfy Nav2 (base_link to odom).
    # Single-robot topic is '/odom'; multi-robot would be '/<ns>/odom' which you can handle later.
    ...
    # odom->tf relay (critical fix for missing TF)
