...
def generate_launch_description():
    ...
    # ----------------- Auto align Gazebo <-> Map + initialpose -----------------
    # Wait for map_server & amcl (active) -> read YAML origin -> publish:
    #   static TF: gazebo_world -> map  =  (-ox, -oy, 0, -oyaw)
    #   /initialpose in map            =  (spawn - origin)
...
def get_map_yaml(node):
    ...
    if fut.result() and fut.result().values:
        s = fut.result().values[0].string_value
...
# 3) static TF gazebo_world -> map (inverse of origin)
subprocess.Popen([
...
# 4) publish /initialpose in map
x_map = SPAWN_X - ox
y_map = SPAWN_Y - oy
yaw_map = SPAWN_YAW - oyaw
...
pub.publish(msg)
n.get_logger().info(f"gazebo_world->map set to (-{ox:.3f}, -{oy:.3f}, yaw {-oyaw:.3f})")
