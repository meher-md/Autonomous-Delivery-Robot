...
def generate_launch_description():
    ...
    # ---------------- Launch Arguments ----------------
    ros_bridge_arg = DeclareLaunchArgument('ros_bridge', default_value='True', description='Run ROS–Gazebo bridge.')
    rviz_arg       = DeclareLaunchArgument('rviz', default_value='True', description='Start RViz.')
    world_name_arg = DeclareLaunchArgument('world_name', default_value='office.sdf', description='World file (should match the map).')
    robots_arg     = DeclareLaunchArgument('robots', default_value="andino={x: 0., y: 0., z: 0.1, yaw: 0.};", description='Robots to spawn; separate multiple with semicolons.')
    gui_config_arg = DeclareLaunchArgument('gui_config', default_value='default.config', description='Gazebo GUI configuration file.')
    nav2_arg       = DeclareLaunchArgument('nav2', default_value='True', description='Enable Nav2.')
    map_name_arg   = DeclareLaunchArgument('map', default_value='office', description='Map folder under andino_gz/maps/<map>/<map>.yaml')
    params_file_arg= DeclareLaunchArgument('params_file', default_value=PathJoinSubstitution([pkg_andino_gz, 'config', 'nav2_params.yaml']), description='Nav2 parameters YAML.')
    auto_initpose_arg = DeclareLaunchArgument('auto_initpose', default_value='True', description='Auto-publish initial pose(s) to AMCL equal to Gazebo spawn.')

    # اختيار نمط محاذاة map<->world
    map_world_align_arg = DeclareLaunchArgument(
        'map_world_align', default_value='use_map_origin',  # أو identity
    ...
    # ---------------- Gazebo ----------------
    gz_args = TextJoin(substitutions=[world_path, TextJoin(substitutions=["--gui-config", gui_config_path], separator=' ')], separator=' ')
    ...
    # --------- Static TF map↔world + نشر تعويض الإزاحة كمتغيّرات بيئية ---------
    def _setup_map_world_tf(context):
        ...
        if align_mode == 'use_map_origin':
            try:
                ...
                return [
                    LogInfo(msg=f"[map_world_align] Failed to read origin from {yaml_path}: {e}"),
                    SetEnvironmentVariable('MAP_WORLD_XOFF', '0.0'),
                    SetEnvironmentVariable('MAP_WORLD_YOFF', '0.0'),
        ...
        return [
            LogInfo(msg=f"[map_world_align] map->world translation = ({x_off:.3f}, {y_off:.3f}, 0)"),
            SetEnvironmentVariable('MAP_WORLD_XOFF', str(x_off)),
            SetEnvironmentVariable('MAP_WORLD_YOFF', str(y_off)),
            Node(package='tf2_ros', executable='static_transform_publisher',
                 name='tf_map_to_world',
    ...
    static_tf_setup = OpaqueFunction(function=_setup_map_world_tf)
    ...
    for robot_name, init_pose in robots_list.items():
        ...
        nav_group = GroupAction(actions=[
            ...
            SetRemap(src='/' + robot_name + '/global_costmap/scan', dst='/' + robot_name + '/scan', condition=IfCondition(PythonExpression([more_than_one, ' and ', nav2_flag]))),
            SetRemap(src='/' + robot_name + '/local_costmap/scan',  dst='/' + robot_name + '/scan', condition=IfCondition(PythonExpression([more_than_one, ' and ', nav2_flag]))),
            ...
            TimerAction(period=nav2_delay, actions=[
                ...
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(os.path.join(pkg_nav2_bringup, 'launch', 'bringup_launch.py')),
                    launch_arguments={'namespace': robot_name, 'use_namespace': 'True', 'map': map_yaml_path, 'autostart': 'True', 'use_sim_time': 'True', 'params_file': params_file}.items(),
                    condition=IfCondition(PythonExpression([more_than_one, ' and ', nav2_flag])),
                ),
            ]),

            # --------- initialpose في إطار map (world -> map) ---------
            TimerAction(period=nav2_delay + 2.0, actions=[
                ExecuteProcess(
                    ...
                    cmd=[
                        ...
                        (
                            ...
                            'rclpy.init(); '
                            f'xw={float(init_pose["x"])}; yw={float(init_pose["y"])}; yaw={float(init_pose["yaw"])}; '
                            'xoff=float(os.getenv("MAP_WORLD_XOFF","0.0")); '
                            'yoff=float(os.getenv("MAP_WORLD_YOFF","0.0")); '
                            'xm=xw - xoff; ym=yw - yoff; '
                            'n=Node("auto_initialpose_%s"); ' % robot_name +
                            'p=n.create_publisher(PoseWithCovarianceStamped,"' + (ns_initialpose_topic if len(robots_list)>1 else '/initialpose') + '",10); '
                            ...
                            'p.publish(m); '
                            'n.get_logger().info(f"[auto_initialpose] world=({xw:.3f},{yw:.3f})  map=({xm:.3f},{ym:.3f})  yaw={yaw:.3f}"); '
                            'rclpy.shutdown()'
                        ),
                    ],
                    output='screen'
            ...
            # clear costmaps بعد ضبط الوضع
            TimerAction(period=nav2_delay + 4.0, actions=[
                ExecuteProcess(cmd=['/bin/bash','-lc','ros2 service call /global_costmap/clear_entirely_global_costmap std_srvs/srv/Empty "{}"'], output='screen'),
                ExecuteProcess(cmd=['/bin/bash','-lc','ros2 service call /local_costmap/clear_entirely_local_costmap std_srvs/srv/Empty "{}"'], output='screen'),
            ]),
        ])
    ...
    # static TF (ويتم تمرير الإزاحة كمتغيرات بيئية لاستخدامها بالـ initialpose)
