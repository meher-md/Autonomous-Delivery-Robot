...
def generate_launch_description():
    ...
    initpose_x_arg = DeclareLaunchArgument('initpose_x', default_value='__spawn__',
                                           description='Initial X in map. "__spawn__" -> use spawn value.')
    initpose_y_arg = DeclareLaunchArgument('initpose_y', default_value='__spawn__',
                                           description='Initial Y in map. "__spawn__" -> use spawn value.')
    ...
    base_group = GroupAction(
        actions=[
            # اجعل RViz/Gazebo يعملان بريندر سوفت داخل الكونتينر
    ...
    for robot_name, spawn in robots_list.items():
        ...
        nav_group = GroupAction(
            actions=[
                ...
                # -------- Auto initialpose (map frame) --------
                # الافتراضي: يستخدم قيم الـ spawn. لو initpose_* != "__spawn__" هيستخدمها بدلاً من ذلك.
                TimerAction(
                    period=10.0,
                    actions=[
                        ExecuteProcess(
                            cmd=[
                                'python3', '-c',
                                (
                                    'import rclpy, math, os, sys; '
                                    'from geometry_msgs.msg import PoseWithCovarianceStamped; '
                                    'from rclpy.node import Node; '
                                    'rclpy.init(); '
                                    'topic=sys.argv[1]; '
                                    'x_spawn=float(sys.argv[2]); '
                                    'y_spawn=float(sys.argv[3]); '
                                    'yaw_spawn=float(sys.argv[4]); '
                                    'x_arg=os.getenv("INITPOSE_X","__spawn__"); '
                                    'y_arg=os.getenv("INITPOSE_Y","__spawn__"); '
                                    'yawd_arg=os.getenv("INITPOSE_YAWD","__spawn__"); '
                                    'x = float(x_arg) if x_arg!="__spawn__" else x_spawn; '
                                    'y = float(y_arg) if y_arg!="__spawn__" else y_spawn; '
                                    'yaw = math.radians(float(yawd_arg)) if yawd_arg!="__spawn__" else yaw_spawn; '
                                    ...
                                    'p.publish(m); '
                                    'n.get_logger().info(f"[auto_initialpose] topic={topic} x={x:.3f} y={y:.3f} yaw={yaw:.3f}rad"); '
                                    'rclpy.shutdown()'
                                ),
                                (ns_initialpose_topic if nrobots > 1 else '/initialpose'),
                                str(spawn['x']), str(spawn['y']), str(spawn['yaw'])
                            ],
                            output='screen',
                            additional_env={
    ...
    # لاحظ: لا ننشر أي static TF على odom ولا نحرّك map بالنسبة لـ world.
