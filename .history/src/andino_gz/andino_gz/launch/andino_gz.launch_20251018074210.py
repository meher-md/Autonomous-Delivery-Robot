...
def generate_launch_description():
    ...
    # تأخير تشغيل Nav2 بعد الجسر
    nav2_delay_arg = DeclareLaunchArgument(
        'nav2_delay', default_value='3.0', description='Delay (sec) before launching Nav2.'
    )
    ...
    # تحسين إخراج اللوج
    buffered_log = SetEnvironmentVariable('RCUTILS_LOGGING_BUFFERED_STREAM', '1')
    ...
    for robot_name in robots_list:
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                # RViz (Nav2 config) - Software GL يقلل مشاكل OpenGL داخل الحاويات
                TimerAction(
                    period=5.0,
                    actions=[
                        Node(
                ...
                # Bridges بين Gazebo و ROS (يشمل odom, tf, laser, camera, imu ...)
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
        ...
        # Nav2 + EKF — مؤجَّل بعد الجسر بثواني
        nav_group = GroupAction(
            ...
            actions=[
                ...
                # EKF: ينشر odom->base_link من /odom و /imu/data
                Node(
                ...
                # Nav2 bringup بعد تأخير nav2_delay
                TimerAction(
                    period=nav2_delay,
                    actions=[
                        IncludeLaunchDescription(
    ...
    auto_initial_pose = TimerAction(
        # انتظر حتى Nav2 + TF يبقوا جاهزين
        period=10.0,
        actions=[
            ExecuteProcess(
                cmd=[
    ...
    clear_costmaps = TimerAction(
        period=12.0,  # ~2s بعد initialpose
        actions=[
            ExecuteProcess(
                cmd=['/bin/bash', '-lc',
    ...
    # TF الثابت فقط (gazebo_world -> map)
