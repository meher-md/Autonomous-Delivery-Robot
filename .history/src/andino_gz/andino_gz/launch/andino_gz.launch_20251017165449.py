...
import os
import math  # <-- for yaw->quaternion
from ament_index_python.packages import get_package_share_directory
from launch import LaunchDescription
from launch.actions import DeclareLaunchArgument, GroupAction, IncludeLaunchDescription, LogInfo, TimerAction  # <-- TimerAction added
...
def generate_launch_description():
    ...
    # Optional: allow toggling auto initial pose and costmap clearing
    ...
    initpose_delay_arg = DeclareLaunchArgument(
        'initialpose_delay', default_value='6.0',
        description='Seconds to wait before publishing initial pose (let AMCL activate).')
    auto_clear_costmaps_arg = DeclareLaunchArgument(
        'auto_clear_costmaps', default_value='True',
        description='Automatically clear global/local costmaps shortly after initial pose.')
    ...
    for robot_name in robots_list:
        ...
        # Pre-compute quaternion (z,w) from yaw (assumed radians)
        yaw = float(init_pose['yaw'])
        qz = math.sin(yaw / 2.0)
        qw = math.cos(yaw / 2.0)

        # Build YAML for /initialpose message using the spawn pose
        initialpose_yaml = (
            f"{{header: {{frame_id: map}}, "
            f"pose: {{pose: {{position: {{x: {init_pose['x']}, y: {init_pose['y']}, z: 0.0}}, "
        ...
        # This group launches the robot, RViz, and the bridge
        robots_group = GroupAction(
            ...
            actions=[
                ...
                PushRosNamespace(
                    condition=IfCondition(more_than_one_one := PythonExpression([more_than_one_robot])),
        ...
        # This group launches Nav2 and, if enabled, auto initial pose + costmap clearing
        nav_group = GroupAction(
          ...
          actions=[
              ...
              SetRemap(src='/' + robot_name + '/global_costmap/scan', dst='/' + robot_name + '/scan', condition=IfCondition(PythonExpression([PythonExpression([TextSubstitution(text=str(len(robots_list.keys()))), ' > 1']), ' and ', LaunchConfiguration('nav2')]))),
              SetRemap(src='/' + robot_name + '/local_costmap/scan', dst='/' + robot_name + '/scan', condition=IfCondition(PythonExpression([PythonExpression([TextSubstitution(text=str(len(robots_list.keys()))), ' > 1']), ' and ', LaunchConfiguration('nav2')]))),
              ...
              IncludeLaunchDescription(
                  ...
                  condition=IfCondition(PythonExpression([PythonExpression([TextSubstitution(text=str(len(robots_list.keys()))), ' > 1']), ' and ', LaunchConfiguration('nav2')])),
              ),
              SetRemap(src='/global_costmap/scan', dst='/scan', condition=IfCondition(PythonExpression([PythonExpression([TextSubstitution(text=str(len(robots_list.keys()))), ' == 1']), ' and ', LaunchConfiguration('nav2')]))),
              SetRemap(src='/local_costmap/scan', dst='/scan', condition=IfCondition(PythonExpression([PythonExpression([TextSubstitution(text=str(len(robots_list.keys()))), ' == 1']), ' and ', LaunchConfiguration('nav2')]))),
              ...
              # --- Auto initial pose (relative topic 'initialpose' works both with and without namespace)
              TimerAction(
              ...
              # --- Optional: clear costmaps shortly after initial pose
              TimerAction(
    ...
    ld.add_action(auto_initpose_arg)       # <-- new
    ld.add_action(initpose_delay_arg)      # <-- new
    ld.add_action(auto_clear_costmaps_arg) # <-- new
