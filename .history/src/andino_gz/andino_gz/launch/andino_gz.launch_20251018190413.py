...
def generate_launch_description():
    ...
    # Allow choosing which world-like frame to parent odom under (use the one that actually exists in your sim).
    ...
    base_group = GroupAction(
        ...
        actions=[
            ...
            # ROS Bridge for /clock (fixed closing bracket)
            Node(
    ...
    # ---------------- Static TF (connect the TF tree) ----------------
    # IMPORTANT (English): Provide exactly one static transform world->odom to avoid TF loops.
    ...
    for robot_name in robots_list:
        ...
        # --- Safe bridges per robot (NO pose->tf) ---
        # English: Use exact GZ topics known from sim; remap odometry to /odom and scan to /scan.
        # Note: world entity name is typically 'gazebo_world' in Ignition; adjust via tf_world_parent if needed.
        ...
        # --- EKF: publishes odom->base_link (inline params, no external files) ---
        ekf_params = {
            'use_sim_time': True,
            'map_frame': 'map',
            'odom_frame': 'odom',
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                # Safe bridge node (replaces any include that might inject pose->tf)
                per_robot_bridge,
                    ...
                    actions=[
                        Node(
                            ...
                            additional_env={
                                ...
                                'QT_XCB_GL_INTEGRATION': 'none',   # safer on headless / SW GL
                                'QT_QPA_PLATFORM': 'xcb'
                            }
                        )
                    ]
    ...
    # Clear costmaps only when Nav2 is enabled (prevents noisy errors on startup)
    clear_costmaps = GroupAction(
        actions=[
            ExecuteProcess(
    ...
    clear_costmaps_timer = TimerAction(
        period=12.0,  # ~2s after initial pose
        actions=[clear_costmaps],
    ...
    # Static TF: ONLY provide world->odom once; do NOT add any odom->world / odom->gazebo_world
