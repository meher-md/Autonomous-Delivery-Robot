...
def generate_launch_description():
    ...
    world_name_arg = DeclareLaunchArgument('world_name', default_value='office.sdf', description='World file.')
    ...
    gui_config_arg = DeclareLaunchArgument('gui_config', default_value='default.config', description='GUI config.')
    nav2_arg       = DeclareLaunchArgument('nav2', default_value='True', description='Enable Nav2.')
    map_name_arg   = DeclareLaunchArgument('map', default_value="office", description='Map name (folder).')
    ...
    initpose_x_arg = DeclareLaunchArgument('initpose_x', default_value='0.0', description='Initial pose X in map.')
    initpose_y_arg = DeclareLaunchArgument('initpose_y', default_value='0.0', description='Initial pose Y in map.')
    initpose_yaw_deg_arg = DeclareLaunchArgument('initpose_yaw_deg', default_value='0.0', description='Initial yaw deg in map.')
    ...
    # ---------------- Static TF (connect gazebo_world -> map) ----------------
    static_tf_gz_to_map = Node(
        package='tf2_ros', executable='static_transform_publisher', name='tf_gazebo_world_to_map',
        arguments=['0','0','0','0','0','0','gazebo_world','map'], output='screen'
    )
    ...
    for robot_name in robots_list:
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                # Robot State Publisher (لازم لتحويلات الروابط/الحساسات من URDF)
                Node(
                ...
                # ---- Small helper: convert /odom (nav_msgs/Odometry) into TF: odom -> base_link ----
                ExecuteProcess(
                    cmd=[
                        'python3','-c',
                        (
                            ...
                            'rclpy.init(); n=Node("odom_to_tf"); br=TransformBroadcaster(n); '
                            'def cb(msg): '
                            '  t=TransformStamped(); t.header.stamp=msg.header.stamp; '
                            '  t.header.frame_id=msg.header.frame_id or "odom"; '
                            '  t.child_frame_id=msg.child_frame_id or "base_link"; '
                            '  p=msg.pose.pose.position; q=msg.pose.pose.orientation; '
                            '  t.transform.translation.x=p.x; t.transform.translation.y=p.y; t.transform.translation.z=p.z; '
                            '  t.transform.rotation=q; br.sendTransform(t); '
                            'n.create_subscription(Odometry,"/odom",cb,10); rclpy.spin(n)'
                        )
                    ],
                    output='screen'
                ),
            ...
            ExecuteProcess(
                cmd=[
                    ...
                    (
                        ...
                        'from geometry_msgs.msg import PoseWithCovarianceStamped; from rclpy.node import Node; '
    ...
    ld.add_action(log_world_path); ld.add_action(log_map_path); ld.add_action(base_group)

    ld.add_action(static_tf_gz_to_map)     # single world root
