...
def generate_launch_description():
    ...
    robots_arg     = DeclareLaunchArgument('robots', default_value="andino={x: 0., y: 0., z: 0.1, yaw: 0.};", description='Robots to spawn (semicolon separated).')
    ...
    params_file_arg= DeclareLaunchArgument('params_file',
                          ...
                          description='Nav2 params file (ROS 2 parameters).')
    # NEW: ultrasonic toggle
    use_ultrasonic_arg = DeclareLaunchArgument('use_ultrasonic', default_value='True', description='Enable/disable ultrasonic sensor in xacro.')
    ...
    gui_cfg_path = PathJoinSubstitution([pkg_andino_gz, 'config_gui', gui_config])
    ...
    gz_args = TextJoin(
        substitutions=[
            world_path,
            TextJoin(substitutions=["--gui-config", gui_cfg_path], separator=' '),
        ],
        separator=' ',
    )

    # ---- Base group: Gazebo + generic bridge
    base_group = GroupAction(
        ...
        actions=[
            ...
            # Clock bridge (modern type mapping)
            Node(
    ...
    for robot_name in robots_list:
        ...
        robots_group = GroupAction(
            ...
            launch_configurations={
                ...
                'ros_bridge': ros_bridge,
                'nav2': nav2_flag,
                'use_ultrasonic': use_ultrasonic,   # <<< ensure availability inside the group
            },
            actions=[
                ...
                # Spawn robot (forward ultrasonic arg to spawn file)
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                    ...
                    launch_arguments={
                        ...
                        'use_sim_time': 'true',
                        'use_ultrasonic': use_ultrasonic,   # pass through
                    }.items(),
                ),
                ...
                # ROS<->GZ bridge (also pass ultrasonic toggle if your include supports it)
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                    ...
                    launch_arguments={
                        'entity': robot_name,
                        'use_ultrasonic': use_ultrasonic,   # pass through to bridge include
                    }.items(),
        ...
        nav_group = GroupAction(
          ...
          actions=[
              # Remap scans when using namespaces with Nav2
    ...
    ld.add_action(use_ultrasonic_arg)   # make the arg available
