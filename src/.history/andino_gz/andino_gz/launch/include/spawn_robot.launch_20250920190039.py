...
Pipeline:
  1) xacro -> /tmp/<entity>.urdf
  2) gz sdf -p /tmp/<entity>.urdf -> /tmp/<entity>.sdf
...
def generate_launch_description():
    ...
    urdf_path  = PathJoinSubstitution([TextSubstitution(text='/tmp/'), entity, TextSubstitution(text='.urdf')])
    sdf_path   = PathJoinSubstitution([TextSubstitution(text='/tmp/'), entity, TextSubstitution(text='.sdf')])
    ...
    # ---------- robot_state_publisher (reads URDF string from file) ----------
    ...
    # ---------- 3) Spawn from SDF file (after SDF is ready) ----------
    ...
    # Order: gen_urdf -> gen_sdf -> spawn
    on_urdf_done = RegisterEventHandler(
        OnProcessExit(target_action=gen_urdf_proc, on_exit=[gen_sdf_proc])
    )
