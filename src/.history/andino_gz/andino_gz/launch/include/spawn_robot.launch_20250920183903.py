...
This version generates URDF from xacro at launch time, passes `use_ultrasonic`,
converts URDF->SDF on the fly, and spawns with `-string` (no temp files needed).
"""
...
def generate_launch_description():
    ...
    # NEW: toggle ultrasonic from parent launch
    ...
    # ---------- Paths ----------
    # NOTE: نستعمل andino_gz.urdf.xacro (زي الأصلي) عشان يضم أجزاء gz،
    # ولو محتاج تغييره لملف andino_description/urdf/andino.urdf.xacro ينفع برضه.
    xacro_file = PathJoinSubstitution([
        FindPackageShare('andino_gz'), 'urdf', 'andino_gz.urdf.xacro'
    ])

    # ---------- xacro -> URDF (نمرر use_ultrasonic + أي مفاتيح مطلوبة) ----------
    urdf_cmd = Command([
        'xacro ', xacro_file,
    ...
    # robot_state_publisher مع نفس الـ URDF
    rsp_node = Node(
