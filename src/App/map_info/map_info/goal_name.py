...
class GoalNameNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('enable_click_input', False)  # default: publish-only
    ...
    def on_click(self, msg: PointStamped):
        ...
        self.touch_mtime()
        # Publish the newly added one immediately
    ...
    def reserve_ids(self, name: str):
        if name not in self.id_map:
            self.id_map[name] = (self.next_id, self.next_id + 1)  # (arrow_id, text_id)
            self.next_id += 2
    ...
    def publish_all(self):
        ...
        # Clear previous markers once
        clear = Marker()
        clear.header.frame_id = self.frame_id
        clear.action = Marker.DELETEALL
        ma.markers.append(clear)

        # Add all markers in the same message
