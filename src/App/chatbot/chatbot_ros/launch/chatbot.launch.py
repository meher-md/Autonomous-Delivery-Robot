...
def generate_launch_description():
    ...
    # Whisper STT - Audio capturer DISABLED by default (background mode)
    # Chatbot only responds when triggered via ROS topics from the app
    whisper_cmd = IncludeLaunchDescription(
        ...
        launch_arguments={
            "launch_audio_capturer": LaunchConfiguration(
                "launch_audio_capturer", default="False"  # Background mode: no mic listening
            ),
            "model_repo": "ggerganov/whisper.cpp",
            "model_filename": "ggml-small.bin",  # Faster transcription
    ...
    llama_cmd = create_llama_launch(
        ...
        n_gpu_layers=33,  # Maximum GPU offload for speed
        n_threads=6,  # Increased threads for faster generation
        n_predict=512,  # Increased for full responses
        model_repo="Qwen/Qwen2.5-Coder-3B-Instruct-GGUF",
        model_filename="qwen2.5-coder-3b-instruct-q4_k_m.gguf",
    )

    # Piper TTS
    piper_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
        ...
        launch_arguments={
            ...
            "model_filename": "en/en_US/lessac/medium/en_US-lessac-medium.onnx",
            "model_config_repo": "rhasspy/piper-voices",
            "model_config_filename": "en/en_US/lessac/medium/en_US-lessac-medium.onnx.json",
        }.items(),
    )
